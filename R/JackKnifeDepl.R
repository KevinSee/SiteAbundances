...
JackKnifeDepl = function(data, group = NULL, grouped=FALSE)
{
  ...
  results = data.frame(N.hat = rep(NA, nrow(data)), N.hat.SE=NA, p.hat=NA, Valid=as.logical(NA), row.names=rownames(data))
  ...
  if(grouped==T) {
    for(group in unique(data$grp)) {
      ...
      if(sum(rowSums(data[data$grp==group,c('p1', 'p2', 'p3')]))>0) {
        ...
        results[match(rownames(subset(data, grp==group)), rownames(results)),] = data.frame(N.hat = N.est, N.hat.SE=N.est.SE, p.hat=p.hat, Valid=valid, row.names=NULL)
    }
  }
  return(results)
}
